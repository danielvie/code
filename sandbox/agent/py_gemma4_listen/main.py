...
os.environ["OPENAI_API_KEY"] = "not-needed"  # vLLM doesn't require an API key
...
def execute_terminal_command(command: str) -> str:
    ...
    print(
        f"{Fore.BLUE}Executing (PowerShell): {Style.BRIGHT}{command}{Style.RESET_ALL}"
    )
    try:
...
class HotPTTGemma:
    def __init__(self):
        ...
        try:
            self.stream = sd.InputStream(
                samplerate=SAMPLE_RATE, channels=CHANNELS, callback=self.audio_callback
            )
        except Exception as e:
            print(f"\n{Fore.RED}Audio Device Error: {e}{Style.RESET_ALL}")
            print(
                f"{Fore.YELLOW}Attempting to list available audio devices:{Style.RESET_ALL}"
            )
            try:
    ...
    def process_with_model(self, audio_path):
        ...
        try:
            ...
            prompt_stage1 = (
                "Transcribe the attached audio exactly. Output only the transcription."
            )
            transcribe_message = HumanMessage(
                content=[
                    ...
                    {
                        "type": "image_url",
                        "image_url": f"data:image/wav;base64,{audio_base64}",
                    },
                ]
            )

            print(
                f"{Fore.GREEN}Starting Phase 1 (Transcription Pipeline)...{Style.RESET_ALL}"
            )
            ...
            print(
                f"{Fore.CYAN}--- Phase 2 (Execution Pipeline) Start ---{Style.RESET_ALL}"
            )
                ...
                res = agent.invoke(
                    {
                        "messages": [
                            ...
                            ("user", f"Task generated from audio: {transcription}"),
                        ]
                    }
                )
                ...
                for message in res["messages"]:
                    if message.type == "ai":
                        if getattr(message, "tool_calls", None):
                            print(
                                f"{Fore.LIGHTBLACK_EX}[Thought]: Calling tool {message.tool_calls[0]['name']}{Style.RESET_ALL}"
                            )
                        if message.content:
                            print(
                                f"{Fore.MAGENTA}Response: {message.content}{Style.RESET_ALL}"
                            )
                    elif message.type == "tool":
                        print(
                            f"{Fore.CYAN}Tool Output: {message.content}{Style.RESET_ALL}"
                        )
    ...
    def run(self):
        ...
        print(
            f"Hold {Fore.YELLOW}Ctrl{Style.RESET_ALL} to Speak. {Fore.RED}Esc{Style.RESET_ALL} to exit."
        )
            ...
            try:
                while True:
                    ...
                    if not keyboard.is_pressed("ctrl") and self.is_recording:
                        ...
                        if audio_file:
                            t = threading.Thread(
                                target=self.process_with_model, args=(audio_file,)
                            )

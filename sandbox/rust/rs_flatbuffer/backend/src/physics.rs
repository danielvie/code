...
fn cart_pole_ode(state: Vector4<f64>, force: f64, p: &Params) -> Vector4<f64> {
    ...
    let theta_acc = (g * sin_t - cos_t * temp) / (L * (4.0 / 3.0 - m * cos_t * cos_t / (M + m)));
...
pub fn compute_lqr_gains(p: &Params) -> Vector4<f64> {
    ...
    let b = SVector::<f64, 4>::new(0.0, (inertia + m * L * L) / denom, 0.0, -(m * L) / denom);
...
fn solve_care(
    ...
    for iter in 0..200 {
        let h_inv = match h.clone().try_inverse() {
            Some(inv) => inv,
            None => {
                log::warn!("CARE: Hamiltonian singular at iter {iter}");
                return None;
            }
        };
    ...
    if p_sym.iter().all(|v| v.is_finite()) {
        Some(p_sym)
    } else {
        log::warn!("CARE: non-finite P after extraction.");
        None
    }
}

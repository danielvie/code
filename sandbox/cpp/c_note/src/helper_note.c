...
void helper_note_clear(NoteData *buffer) {
    ...
    memset(buffer->buffer, 0, helper_note_FIXED_SIZE);
    buffer->idx = 0;
...
void helper_note_add_text(NoteData *buffer, const char *text_to_add) {
    if (buffer == NULL || buffer->buffer == NULL || text_to_add == NULL) {
        return;
    }
    if (buffer->is_full) {
        return;
    ...
    size_t remaining_space = (helper_note_FIXED_SIZE > buffer->idx) ? (helper_note_FIXED_SIZE - buffer->idx) : 0;
    ...
    if (buffer->idx < helper_note_FIXED_SIZE) {
...
void helper_note_add_text_f(NoteData *buffer, const char *format, ...) {
    if (buffer == NULL || buffer->buffer == NULL || format == NULL) {
        return;
    }
    if (buffer->is_full) {
    ...
    helper_note_add_text(buffer, temp_buffer);
}

// Add array
void helper_note_add_array_u32(NoteData *buffer, const char *name, const uint32_t *data, size_t data_len) {
    if (buffer == NULL || buffer->buffer == NULL || name == NULL) {
        return;
    }
    if (data_len == 0) {
        helper_note_add_text_f(buffer, "%s: [];\n", name);
        return;
    }

    helper_note_add_text_f(buffer, "%s: [", name);
    for (size_t i = 0; i < data_len; ++i) {
        if (i + 1 < data_len) {
            helper_note_add_text_f(buffer, "%u,", (unsigned)data[i]);
        } else {
            helper_note_add_text_f(buffer, "%u];\n", (unsigned)data[i]);
        }
    }
}
...
void helper_note_add_array_i32(NoteData *buffer, const char *name, const int32_t *data, size_t data_len) {
    ...
    for (size_t i = 0; i < data_len; ++i) {
        if (i + 1 < data_len) {
            helper_note_add_text_f(buffer, "%d,", (int)data[i]);
        } else {
            helper_note_add_text_f(buffer, "%d];\n", (int)data[i]);
        }
    }
}
...
void helper_note_print(const NoteData *buffer) {
    ...
    if (buffer->idx < helper_note_FIXED_SIZE && buffer->buffer[buffer->idx] == '\0') {

...
parser.add_argument(
    "-bd", "--birth_day", help="What is your birth day? Eg. 15", type=int
)
parser.add_argument(
    "-bm", "--birth_month", help="What is your birth day? Eg. 03", type=int
)
...
resource = Resource(
    attributes={
        "service.name": "daniel-test-otel-with-python",
        "os-version": 1234.56,
        "cluster": "A",
        "datacentre": "BNE",
    }
...
processor = BatchSpanProcessor(
    OTLPSpanExporter(
        endpoint=f"http://{COLLECTOR_ENDPOINT}:{COLLECTOR_GRPC_PORT}", insecure=True
    )
)
...
def start_span(name: str) -> Any:
    return cast(Any, tracer.start_as_current_span(name))
...
if __name__ == "__main__":
    with start_span("do-calculations"):
        result = 7
